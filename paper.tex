...
On the data side, Djuhera et al.~\cite{datasalgorithm2025} argue that the dataset \emph{is} the algorithm---that DPO outcomes depend more on how pairs are constructed than on optimizer details.
Ivison et al.~\cite{unpackingdpo2024} and Pan et al.~\cite{whatmatters2025} provide supporting evidence from different angles: sampling strategy and chosen-response quality, respectively.
...
We evaluate Qwen3-8B~\cite{qwen3_2025} on MATH-500~\cite{hendrycks2021math}.
For each of the 500 problems, we generate $N = 8$ rollouts using vLLM~\cite{kwon2023vllm} with temperature $T = 0.7$.
...
Each condition is compared against its base model using McNemar's test~\cite{mcnemar1947}, which counts problems where exactly one model is correct and tests whether those disagreements are one-sided; Dem\v{s}ar~\cite{demsar2006} recommends this test for paired classifier comparisons on shared evaluation sets.
Within each experiment we apply Benjamini-Hochberg correction~\cite{benjamini1995} to control the false discovery rate across multiple comparisons.
...
All experiments use Qwen3-8B with QLoRA~\cite{dettmers2023qlora} (LoRA~\cite{hu2022lora}, $r = 64$, $\alpha = 128$).
...
Pan et al.~\cite{whatmatters2025} argue that chosen-response quality is what drives DPO gains; rollout pairs satisfy that without the distributional artifacts we see in corrections.
...
None exceeded a position-only baseline (AUROC, area under the receiver operating characteristic curve, $= 0.901$).
